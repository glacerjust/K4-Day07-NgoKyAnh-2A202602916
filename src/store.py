...
class EmbeddingStore:
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        for r in records:
            ...
            norm_r = sum(x * x for x in emb) ** 0.5
            if norm_q == 0.0 or norm_r == 0.0:
                score = 0.0
            else:
                score = _dot(query_emb, emb) / (norm_q * norm_r)
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        if self._use_chroma and self._collection is not None:
            ids = [getattr(d, "id", str(i + self._next_index)) for i, d in enumerate(docs)]
            ...
            # Keep in-memory sync for tests just in case
            for d in docs:
