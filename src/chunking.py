...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Split on ". ", "! ", "? " or ".\n"
        # We capture the separator to keep the sentence structure mostly intact or discard it?
        # The simplest approach is to use re.split with a capture group.
        parts = re.split(r'([.!?]\s+|\.\n)', text)
        sentences = []
        current = ""
        for p in parts:
        ...
        for s in splits:
            if len(s) > self.chunk_size:
                if current_chunk:
                    ...
                    current_chunk = ""
                chunks.extend(self._split(s, remaining_separators[1:]))
            else:
                join_len = len(sep) if current_chunk else 0
                if len(current_chunk) + join_len + len(s) <= self.chunk_size:
                    if current_chunk:
                        current_chunk += sep + s
                    else:
                        current_chunk = s
